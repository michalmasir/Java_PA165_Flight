...
public class FlightServiceTest  extends AbstractTestNGSpringContextTests {
    ...
    public void testGetFlights() {
        ...
        assertEquals(flightTOs.get(0).getId(), flights.get(0).getId());
        assertEquals(flightTOs.get(1).getId(), flights.get(1).getId());
    ...
    public void testFlightUpdate() {
        ...
        verify(flightDAO).updateFlight(flight1);
    }
    ...
    public void testFlightGetById() {
        ...
        when(flightDAO.getFlightById(id)).thenReturn(flight1);
        FlightTO flightTO = flightService.getFlightById(id);
        verify(flightDAO).getFlightById(id);

        assertEquals(id, flightTO.getId());
    }
    ...
    public void testFlightGetDepartureByDate() {
        ...
        when(flightDAO.getFlightsByDepartureDate(from.getTime(), to.getTime())).thenReturn(flights);

        List<FlightTO> flightTOs = flightService.getFlightsByDepartureDate(from.getTime(), to.getTime());
        verify(flightDAO).getFlightsByDepartureDate(from.getTime(), to.getTime());
    ...
    public void testFlightGetArrivalByDate() {
        ...
        when(flightDAO.getFlightsByArrivalDate(from.getTime(), to.getTime())).thenReturn(flights);

        List<FlightTO> flightTOs = flightService.getFlightsByArrivalDate(from.getTime(), to.getTime());
        verify(flightDAO).getFlightsByArrivalDate(from.getTime(), to.getTime());
    ...
    public void testAssignSteward(){
       when(flightDAO.safeAddSteward(flight1, steward)).thenReturn(true);
       boolean result =  flightService.assignStewardToFlight(flightTO1, stewardTO);
       verify(flightDAO).safeAddSteward(flight1, steward);
